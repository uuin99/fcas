...
Ext.onReady(function(){
    ...
    function fnSaveForm(){
    	...
    	// 처리 완료일때는 반드시.. 장애처리일시 필수입력 을 체크한다.
    	if(Ext.getCmp('FAIL_STAT').getValue() == '30') {
    		var retStr = null;
	    	if(Ext.getCmp('SHOW_FINS_DT').getValue() == null || Ext.getCmp('SHOW_FINS_DT').getValue() == ''){
	    		retStr = parent.msgProperty.COM_ERR_0014.replace('param1', '장애처리일시'); //param1은(는) 필수입력 사항입니다.//'장애발생일시는 필수 입력 사항입니다.';
	    	}else if(Ext.getCmp('SHOW_FINS_SI').getValue() == null || Ext.getCmp('SHOW_FINS_SI').getValue() == ''){
	    		retStr = parent.msgProperty.COM_ERR_0014.replace('param1', '장애처리일시'); //param1은(는) 필수입력 사항입니다.//'장애발생일시는 필수 입력 사항입니다.';
	    	}else if(Ext.getCmp('SHOW_FINS_BN').getValue() == null || Ext.getCmp('SHOW_FINS_BN').getValue() == ''){
	    		retStr = parent.msgProperty.COM_ERR_0014.replace('param1', '장애처리일시'); //param1은(는) 필수입력 사항입니다.//'장애발생일시는 필수 입력 사항입니다.';
	    	}	    	if(retStr){
		    	fnShowMessage(retStr);
		    	return;
	    	}
	    }

    	// 어떤 경우든지 장애처리 일시가 입력되어있다면 장애발생일시와 비교하여 벨리데이션 체크한다.
    	var failDate = Ext.getCmp('FAIL_DATE').getValue();
    	var finsDate = Ext.Date.format(Ext.getCmp('SHOW_FINS_DT').getValue(), 'Ymd');
    	if(finsDate != '') {
	    	if( failDate > finsDate) {
	    		//retStr = '장애처리일시는 장애발생일시 보다 이후여야 합니다.';
	    		fnShowMessage(parent.msgProperty.COM_ERR_0032.replace('param1', '장애처리일시').replace('param2', '장애발생일시'));
	    		return;
	    	}
    	}
    	...
    	var frm = form.getForm();
    	frm.submit({
    		url: '/com/svc/FailMngt/updateFailFins.do',
    ...
    //	}else if(Ext.getCmp('TREAT_CTS').getValue() == null || Ext.getCmp('TREAT_CTS').getValue() == ''){
    //		retStr = parent.msgProperty.COM_ERR_0014.replace('param1', '장애처리내용'); //param1은(는) 필수입력 사항입니다.//'장애처리내용은 필수 입력 사항입니다.';
    //	}
    //
    //	if(Ext.getCmp('FAIL_STAT').getValue() == '30') {
	//    	if(Ext.getCmp('SHOW_FINS_DT').getValue() == null || Ext.getCmp('SHOW_FINS_DT').getValue() == ''){
	...
	//	    	if( failDate > finsDate) {
	//	    		//retStr = '장애처리일시는 장애발생일시 보다 이후여야 합니다.';
	//	    		retStr =  parent.msgProperty.COM_ERR_0032.replace('param1', '장애처리일시').replace('param2', '장애발생일시')  ;
	//	    	}
	//    	}
    //	}
    //
