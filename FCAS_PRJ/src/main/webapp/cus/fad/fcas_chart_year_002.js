...
Ext.onReady(function(){
    ...
    function fnSetStore(div){
    	...
    	if (div == '1' || div == '5') { //월별-유동인구수
    		categoryField = 'STND_YYMM';
    		fields = ['TOTAL_CNT'];
    		displayNames = ['유동인구수'];
    		modelFields = ['STND_YYMM', 'TOTAL_CNT'];
    		keyFields = ['STND_YYMM'];
    		sumFields = ['TOTAL_CNT'];
    	...
    	} else if (div == '2' || div == '6') { //월별-성별 유동인구수
    		categoryField = 'STND_YYMM';
    		//공통코드 - 성별
    		fields[0] = 'SUM_CNT';
    		displayNames[0] = '합계';
    	...
    	} else if (div == '3' || div == '7') { //월별-연령대별 유동인구수
    		categoryField = 'STND_YYMM';
    		//공통코드 - 연령대별
    	...
    	} else if (div == '4' || div == '8') { //월별-설치위치별 유동인구수
    		categoryField = 'STND_YYMM';
    		//카메라그룹
    ...
    store = Ext.create('Ext.data.JsonStore', {
		...
		proxy: {
			type: 'ajax',
			url: '/cus/fad/YearChart/selectYearChartNew.do',
			reader: {
				type: 'json',
				root: 'data',
				totalProperty: 'total',
    ...
    var combo_chart_div = Ext.create('Ext.form.field.ComboBox', {
        ...
        listeners: {
        	change: function(combo){
        		...
        		if (combo.getValue() == '1' || combo.getValue() == '5') { //월별-유동인구수
        			chartTypeStore.filter([
                       {property: 'TYPE', value: '1'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Column2D');
            	} else if (combo.getValue() == '2' || combo.getValue() == '6') { //월별-성별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '3'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Combination2D');
            	} else if (combo.getValue() == '3' || combo.getValue() == '7') { //월별-연령대별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '1'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Line2D');
            	} else if (combo.getValue() == '4' || combo.getValue() == '8') { //월별-설치위치별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '1'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Line2D');
