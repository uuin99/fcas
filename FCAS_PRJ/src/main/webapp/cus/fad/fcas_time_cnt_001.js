...
Ext.define('TIME_CNT', {
    ...
    fields: ['ROWNUM',
             ...
             'CMRA_GRP_NM','STND_DATE','TIME_CD','TIME_CD_NM','DOV_CD','DOV_CD_NM','TIME_GRP','TIME_GRP_NM',
             'STND_YYMM','ENTER_CNT','EXIT_CNT','TOTAL_CNT','STAY_CNT','WORK_DT',
             'TOTAL_YN', 'WORK_YN']
});
...
Ext.onReady(function(){
    ...
    var store = Ext.create('Ext.data.JsonStore', {
		...
		proxy: {
			type: 'ajax',
			api: {
				read: '/cus/fad/DataCnt/selectTimeCntList.do'
			},
			reader: {
				type: 'json',
				root: 'data',
    	...
    	handler: function(){
    		...
    		store.load({
    		    ...
    		    callback: function(records, operation, success) {
    		        if (success) {
    		    		var radioVal = Ext.getCmp('s_radio_value').getValue().s_radio_value1;
    					var colVal = "COMP_ID,COMP_NM,SHOP_ID,SHOP_NM,CMRA_GRP_ID,CMRA_GRP_NM,TOTAL_YN,STND_DATE,STND_YYMM,WORK_YN,TIME_CD,TIME_CD_NM,DOV_CD,DOV_CD_NM,ENTER_CNT"/*,EXIT_CNT,TOTAL_CNT,STAY_CNT,WORK_DT"*/;
    					var titalVal = "고객사ID,고객사명,매장ID,매장명,카메라그룹ID,카메라그룹명,집계여부,기준일자,기준월,휴무여부,시간대코드,시간대코드명,요일코드,요일코드명,입장객수"/*,퇴장객수,전체객수,평균체류고객수,작업일시"*/
    					if(radioVal == 'TT'){
    						colVal = "COMP_ID,COMP_NM,SHOP_ID,SHOP_NM,STND_DATE,STND_YYMM,WORK_YN,TIME_CD,TIME_CD_NM,DOV_CD,DOV_CD_NM,ENTER_CNT"/*,EXIT_CNT,TOTAL_CNT,STAY_CNT,WORK_DT"*/;
    						titalVal = "고객사ID,고객사명,매장ID,매장명,기준일자,기준월,휴무여부,시간대코드,시간대코드명,요일코드,요일코드명,입장객수"/*,퇴장객수,전체객수,평균체류고객수,작업일시"*/
    					}
    		    		var excelFormObjs = {
    		    			    hiddenType:[
    		    			        ...
    		    			        { "NAME":"SQLID", 		"VALUE": "datacnt.selectTimeCntList" },
    		    			        ...
    		    			        { "NAME":"PROG_ID", 	"VALUE": "fcas_time_cnt_001" },
