...
Ext.onReady(function() {
    ...
    var store = Ext.create('Ext.data.JsonStore', {
		...
		proxy: {
			type: 'ajax',
			api: {
				read: '/cus/fad/DataCls/selectDailyClsList.do'
			},
			reader: {
				type: 'json',
				root: 'data',
    	...
    	handler: function(){
    		...
    		store.load({
    		    ...
    		    callback: function(records, operation, success) {
    		        if (success) {
    					var radioVal = Ext.getCmp('s_radio_value').getValue().s_radio_value1;
    					var colVal = "COMP_ID,COMP_NM,SHOP_ID,SHOP_NM,CMRA_GRP_ID,CMRA_GRP_NM,TOTAL_YN,STND_DATE,STND_YYMM,WORK_YN,DOV_CD,DOV_NM,SEX_CD,SEX_NM,AGE_CD,AGE_NM,ENTER_CNT"/*,WORK_DT"*/;
    					var titalVal = "고객사ID,고객사명,매장ID,매장명,카메라그룹ID,카메라그룹명,집계여부,기준일자,기준월,휴무여부,요일코드,요일코드명,성별코드,성별코드명,연령대코드,연령대코드명,입장객수"/*,작업일시"*/;
    					if(radioVal == 'TT'){
    						colVal = "COMP_ID,COMP_NM,SHOP_ID,SHOP_NM,STND_DATE,STND_YYMM,WORK_YN,DOV_CD,DOV_NM,SEX_CD,SEX_NM,AGE_CD,AGE_NM,ENTER_CNT"/*,WORK_DT"*/;
    						titalVal = "고객사ID,고객사명,매장ID,매장명,기준일자,기준월,휴무여부,요일코드,요일코드명,성별코드,성별코드명,연령대코드,연령대코드명,입장객수"/*,작업일시"*/;
    					}
    		    		var excelFormObjs = {
    		    			    hiddenType:[
    		    			        { "NAME":"SHEET_NAME", 	"VALUE": "일자별 성별_연령별수" },
    		    			        { "NAME":"COLS", 		"VALUE":  colVal},
    		    			        { "NAME":"TITLE", 		"VALUE":  titalVal},
    		    			        { "NAME":"SQLID", 		"VALUE": "datacls.selectListDailyCls" },
    		    			        ...
    		    			        { "NAME":"PROG_ID", 	"VALUE": "fcas_daily_cls_001" },
