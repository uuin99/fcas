/*var chartDivStore = Ext.create('Ext.data.JsonStore', {
    fields: ['CD', 'CD_DESC'],
    data: [
       {CD: '1', CD_DESC: '시간대별-유동인구수'}, //Column2D Scroll(13)
       ...
       {CD: '5', CD_DESC: '시간대별-설치위치별 유동인구수'}, //Line2D Scroll(13)
...
Ext.onReady(function(){
    ...
    function fnSetStore(div){
    	...
    	if (div == '1') { //시간대별-유동인구수
    		...
    		layoutStr = fnGetLayoutCombination2D(charTitle, categoryField, fields, displayNames, false, true, '13');
    	} else if (div == '5') { //시간대별-설치위치별 유동인구수
    		categoryField = 'TIME_NM';
    		//카메라그룹
    ...
    store = Ext.create('Ext.data.JsonStore', {
		...
		proxy: {
			type: 'ajax',
			url: '/cus/fad/DayChart/selectDayChartNew.do',
			reader: {
				type: 'json',
				root: 'data',
				totalProperty: 'total',
        ...
        listeners: {
        	change: function(combo){
        		...
        		if (combo.getValue() == '1') { //시간대별-유동인구수
        			...
        			Ext.getCmp('combo_chart_type').setValue('Combination2D');
            	} else if (combo.getValue() == '5') { //시간대별-설치위치별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '2'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Line2D_S');
