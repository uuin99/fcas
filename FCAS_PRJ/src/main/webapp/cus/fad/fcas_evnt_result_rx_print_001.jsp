...
<title>레포트 출력</title>
	...
	function fnOpen(){
		...
		oReport.rptname = "http://" + location.host + "/report/cus/fad/fcas_evnt_result_rx_print.reb";
		
		oReport.con("XML1").type = "http";
		oReport.con("XML1").datatype = "xml";
		oReport.con("XML1").path = "http://" + location.host + "/cus/fad/SalesInfo/selectEvntResultPrintXml.do?rxParam="+parent.rxParam;
		oReport.con("XML1").dataset("XMLDS1").xpath = "gubun/rpt1/rexdataset/rexrow";
		
		var param = parent.rxParam.split('|');
		var cond1 = param[2].replace('STND_DATE_F:','');
		var cond2 = param[3].replace('STND_DATE_T:','');
		oReport.param("COND").value = '( 기간: '
									+ cond1.substring(0,4)+'년 '+cond1.substring(4,6)+'월 '+cond1.substring(6,8)+'일'
									+ ' ~ '
									+ cond2.substring(0,4)+'년 '+cond2.substring(4,6)+'월 '+cond2.substring(6,8)+'일'
									+ ' )';
...
<body onload="fnOpen();" style="width:100%; height:100%; padding:0px; margin:0px;">
	<iframe name="ifrmRexPreview1" id="ifrmRexPreview1" src='/RexServer30/rexpreview.jsp'  style="width:816px; height:1180px; padding:0px; border:1px; border-style:solid; border-color:#99BCE8;"></iframe>
	<script type="text/javascript">
