/*var chartDivStore = Ext.create('Ext.data.JsonStore', {
    fields: ['CD', 'CD_DESC'],
    data: [
       {CD: '1', CD_DESC: '시간대별-평균 유동인구수'}, //Column2D Scroll(13)
       ...
       {CD: '5', CD_DESC: '시간대별-요일별 평균 유동인구수'}, //Line2D Scroll(13)
       {CD: '6', CD_DESC: '시간대별-설치위치별 평균 유동인구수'}, //Line2D Scroll(13)
       {CD: '7', CD_DESC: '일별-유동인구수'}, //Column2D Scroll(10)
       ...
       {CD: '11', CD_DESC: '일별-설치위치별 유동인구수'}, //Line2D Scroll(10)
       {CD: '12', CD_DESC: '요일별-평균 유동인구수'}, //Pie2D, Column2D
...
function chartClickHanlder(seriesId, displayText, index, data, values){
	...
	if (Ext.getCmp('combo_chart_div').getValue() == '19') { //성별-연령대별 유동인구수 (2차트)
	...
	} else if (Ext.getCmp('combo_chart_div').getValue() == '20') { //연령대별-성별 유동인구수 (2차트)
...
Ext.onReady(function(){
    ...
    function fnSetStore(div){
    	...
    	if (div == '1') { //시간대별-유동인구수
    		...
    		fields = ['TOTAL_CNT'];
    		displayNames = ['유동인구수'];
    		modelFields = ['TIME_CD', 'TIME_NM', 'TOTAL_CNT'];
    		keyFields = ['TIME_CD'];
    		sumFields = ['TOTAL_CNT'];
    		...
    		modelFields = ['TIME_CD', 'TIME_NM', 'TOTAL_CNT', 'STAY_CNT'];
    		keyFields = ['TIME_CD'];
    		...
    		layoutStr = fnGetLayout(charTitle, true, type, categoryField, fields, displayNames, '13');
    	} else if (div == '6') { //시간대별-설치위치별 유동인구수
    		categoryField = 'TIME_NM';
    		//카메라그룹
    		...
    		layoutStr = fnGetLayout(charTitle, true, type, categoryField, fields, displayNames, '10');
    	} else if (div == '16') { //설치위치별-연령대별 유동인구수
    		categoryField = 'CMRA_GRP_NM';
    		//공통코드 - 연령대별
    		for (var i=0; i<ageCdStore.getCount(); i++) {
				fields[i] = ageCdStore.getAt(i).get('CD')+'_CNT';
    		...
    		layoutStr = fnGetLayout(charTitle, true, type, categoryField, fields, displayNames, '');
    	} else if (div == '19') { //성별-연령대별 유동인구수 (2차트)
    		categoryField = 'SEX_NM';
    		fields = ['TOTAL_CNT'];
			sumFields = ['TOTAL_CNT'];
			sumAliases = ['TOTAL_CNT'];
    	...
    	} else if (div == '20') { //연령대별-성별 유동인구수 (2차트)
    		categoryField = 'AGE_NM';
    		fields = ['TOTAL_CNT'];
			sumFields = ['TOTAL_CNT'];
			sumAliases = ['TOTAL_CNT'];
    ...
    store = Ext.create('Ext.data.JsonStore', {
		...
		proxy: {
			type: 'ajax',
			url: '/cus/fad/TermChart/selectTermChartNew.do',
			reader: {
				type: 'json',
				root: 'data',
				totalProperty: 'total',
        ...
        listeners: {
        	change: function(combo){
        		...
        		if (combo.getValue() == '1') { //시간대별-유동인구수
        			...
        			Ext.getCmp('combo_chart_type').setValue('Combination2D');
            	} else if (combo.getValue() == '5') { //시간대별-요일별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '2'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Line2D_S');
            	} else if (combo.getValue() == '6') { //시간대별-설치위치별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '2'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Line2D_S');
            	} else if (combo.getValue() == '7') { //일별-유동인구수
        			chartTypeStore.filter([
                       {property: 'TYPE', value: '2'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Column2D_S');
            	} else if (combo.getValue() == '8') { //일별-성별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '3'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Combination2D');
            	} else if (combo.getValue() == '9') { //일별-연령대별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '2'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Line2D_S');
        			...
        			Ext.getCmp('combo_chart_type').setValue('Combination2D');
            	} else if (combo.getValue() == '11') { //일별-설치위치별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '2'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Line2D_S');
            	} else if (combo.getValue() == '12') { //요일별-유동인구수
            	...
            	} else if (combo.getValue() == '13') { //요일별-성별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '3'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Combination2D');
            	} else if (combo.getValue() == '14') { //요일별-연령대별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '1'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Line2D');
            	} else if (combo.getValue() == '15') { //설치위치별-성별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '2'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Column2D_S');
            	} else if (combo.getValue() == '16') { //설치위치별-연령대별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '2'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Column2D_S');
            	} else if (combo.getValue() == '17') { //성별-연령대별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '1'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Column2D');
            	} else if (combo.getValue() == '18') { //연령대별-성별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '1'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Column2D');
            	} else if (combo.getValue() == '19') { //성별-연령대별 유동인구수 (2차트)
            		fnSetDivDisplay('2Chart');
        			...
        			Ext.getCmp('combo_chart_type').setValue('2Chart');
            	} else if (combo.getValue() == '20') { //연령대별-성별 유동인구수 (2차트)
            		fnSetDivDisplay('2Chart');
