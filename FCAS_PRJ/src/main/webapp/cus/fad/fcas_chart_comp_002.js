...
Ext.onReady(function(){
    ...
    function fnSetStore(div){
    	...
    	if (div == '1') { //입장/체류객수
    		...
    		fields = ['TOTAL_CNT'];
    		displayNames = ['유동인구수'];
    		modelFields = ['DIV_NM', 'TOTAL_CNT'];
    		keyFields = ['DIV_NM'];
    		sumFields = ['TOTAL_CNT'];
    		sumAliases = ['TOTAL_CNT'];
    		/*fields = ['TOTAL_CNT', 'STAY_CNT'];
    		displayNames = ['유동인구수', '체류객수'];
    		modelFields = ['DIV_NM', 'TOTAL_CNT', 'STAY_CNT'];
    		keyFields = ['DIV_NM'];
    		...
    		layoutStr = fnGetLayout(charTitle, true, type, categoryField, fields, displayNames);
    	} else if (div == '4') { //시간대별 유동인구수
    		categoryField = 'TIME_NM';
    		fields = ['TOTAL_CNT1', 'TOTAL_CNT2'];
    		displayNames = ['기준', '비교'];
    		modelFields = ['TIME_CD', 'TIME_NM', 'TOTAL_CNT1', 'TOTAL_CNT2'];
    		sumFields = ['TOTAL_CNT', 'TOTAL_CNT'];
    		sumAliases = ['TOTAL_CNT1', 'TOTAL_CNT2'];
    		if (cmra_grp_id == '') {
    			Ext.apply(
					extraParams,
					{ CMRA_GRP_ID: 'TT' }
    ...
    store = Ext.create('Ext.data.JsonStore', {
		...
		proxy: {
			type: 'ajax',
			url: '/cus/fad/CompChart/selectCompChartNew.do',
			reader: {
				type: 'json',
				root: 'data',
				totalProperty: 'total',
    ...
    var combo_chart_div = Ext.create('Ext.form.field.ComboBox', {
        ...
        listeners: {
        	change: function(combo){
        		...
        		if (combo.getValue() == '1') { //입장/체류객수
            	...
            	} else if (combo.getValue() == '2') { //성별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '1'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Column2D');
            	} else if (combo.getValue() == '3') { //연령대별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '1'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Column2D');
            	} else if (combo.getValue() == '4') { //시간대별 유동인구수
            		chartTypeStore.filter([
        			...
        			Ext.getCmp('combo_chart_type').setValue('Line2D_S');
            	} else if (combo.getValue() == '6') { //시간대별-성별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '2'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Line2D_S');
            	} else if (combo.getValue() == '7') { //시간대별-연령대별 유동인구수
            		chartTypeStore.filter([
                       {property: 'TYPE', value: '2'}
                    ]);
        			Ext.getCmp('combo_chart_type').setValue('Line2D_S');
