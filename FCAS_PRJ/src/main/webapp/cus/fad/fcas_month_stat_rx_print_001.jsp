...
<title>레포트 출력</title>
	...
	function fnOpen(){
		...
		oReport.rptname = "http://" + location.host + "/report/cus/fad/fcas_month_stat_rx_print.reb";
		
		oReport.con("XML1").type = "http";
		oReport.con("XML1").datatype = "xml";
		oReport.con("XML1").path = "http://" + location.host + "/cus/fad/SalesInfo/selectMonthStatPrintXml.do?rxParam="+parent.rxParam;
		oReport.con("XML1").dataset("XMLDS1").xpath = "gubun/rpt1/rexdataset/rexrow";
		
		var param = parent.rxParam.split('|');
		var cond = param[2].replace('STND_YYYY:','');
		oReport.param("COND").value = '( 기준년 : ' + cond+'년 )';
