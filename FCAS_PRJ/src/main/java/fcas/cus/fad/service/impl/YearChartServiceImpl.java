...
/**
 * @logicalName   Year Chart Service Impl.
 * @description
 * @version       $Rev: 1.0 $
 */
@Service("yearchartService")
public class YearChartServiceImpl extends AbstractServiceImpl implements YearChartService {

    @Resource(name="commonDAO")
    private CommonDAO commonDAO;

    /**
	 * Year Chart Query를 생성한다.
	 * @param model
    ...
    private StringBuilder getChartQuery(Map<String, Object> model){
		...
		String fromTable = "";
    	if (sDiv.equals("1") || sDiv.equals("4")       //월별-입장객수, 월별-설치위치별 입장객수
    		|| sDiv.equals("5") || sDiv.equals("8")) { //월별-평균입장객수, 월별-설치위치별 평균입장객수
		...
		for (int i=0; i<keyFieldArray.length; i++) {
			if (keyFieldArray[i].equals("STND_YYMM")) {
				sQuery.append("SUBSTRING(STND_YYMM,5,2)+'월' AS 'STND_YYMM', ");
			} else {
		...
		for (int i=0; i<sumFieldArray.length; i++) {
			if (sDiv.equals("1") || sDiv.equals("2") || sDiv.equals("3") || sDiv.equals("4")) {
				if (caseField.equals("")) {
		...
		for (String key : model.keySet()) {
			if (!model.get(key).toString().equals("")) {
				...
				if (tmpStr.length > 1) {
					...
					if (key.equals("STND_YY")) {
						sQuery.append(" AND STND_DATE LIKE '" + model.get(key).toString() + "%' ");
					} else {
		...
		sQuery.append("         AND STND_DATE LIKE '"+model.get("STND_YY").toString()+"%' ");
	 ...
	 * @exception
	 */
    public ModelAndView selectYearChart(Map<String, Object> model) throws Exception {
	 ...
	 * @exception
	 */
    public ModelAndView selectYearChartNew(Map<String, Object> model) throws Exception {
    	ArrayList<HashMap<String, Object>> tmpList = new ArrayList<HashMap<String, Object>>();
    	Connection conn = null;
    	Statement stmt = null;
    	try {
...
//		sQuery.append("         AND WORK_YN   = 'Y') ");
	 ...
	 * @exception
	 */
    public ModelAndView selectYearChartCpa(Map<String, Object> model) throws Exception {
    	ArrayList<HashMap<String, Object>> tmpList = new ArrayList<HashMap<String, Object>>();
    	Connection conn = null;
    	Statement stmt = null;
    	try {
