...
/**
 * @logicalName   Term Chart Service Impl.
 * @description
 * @version       $Rev: 1.0 $
 */
@Service("termchartService")
public class TermChartServiceImpl extends AbstractServiceImpl implements TermChartService {

    @Resource(name="commonDAO")
    private CommonDAO commonDAO;

    /**
	 * Term Chart Query를 생성한다.
	 * @param model
    ...
    private StringBuilder getChartQuery(Map<String, Object> model){
		...
		String fromTable = "";
    	if (sDiv.equals("1") || sDiv.equals("4")       //시간대별-입장객수, 시간대별-체류고객수
    		|| sDiv.equals("5") || sDiv.equals("6")) { //시간대별-요일별 입장객수, 시간대별-설치위치별 입장객수
    		fromTable = "FCA_TIMES_STATS_CNT";
    	} else if (sDiv.equals("2") || sDiv.equals("3")) {  //시간대별-성별 입장객수, 시간대별-연령대별 입장객수
    		fromTable = "FCA_TIMES_STATS_CLASS";
    	} else if (sDiv.equals("7") || sDiv.equals("10") || sDiv.equals("11") //일별-입장객수, 일별-평균체류고객수, 일별-설치위치별 입장객수
    		|| sDiv.equals("12")) { //요일별-입장객수
		...
		for (int i=0; i<keyFieldArray.length; i++) {
			if (keyFieldArray[i].equals("STND_DATE")) {
				sQuery.append("SUBSTRING(STND_DATE,3,2)+'/'+SUBSTRING(STND_DATE,5,2)+'/'+SUBSTRING(STND_DATE,7,2) AS 'STND_DATE', ");
			} else {
				sQuery.append(keyFieldArray[i] + " , ");
			}

			/* 시간대별, 성별, 연령대별, 요일별, 설치위치별, 이벤트별 */
			if (keyFieldArray[i].equals("TIME_CD")) {
				sQuery.append("dbo.FN_CODE_NM('time_cd',TIME_CD) AS 'TIME_NM', ");
			} else if (keyFieldArray[i].equals("SEX_CD")) {
		...
		for (int i=0; i<sumFieldArray.length; i++) {
			if (sDiv.equals("7") || sDiv.equals("8") || sDiv.equals("9")
				|| sDiv.equals("10") || sDiv.equals("11")) { //일별
				if (caseField.equals("")) {
		...
		for (String key : model.keySet()) {
			if (!model.get(key).toString().equals("")) {
				...
				if (tmpStr.length > 1) {
					...
					if (key.equals("STND_DATE_F")) {
						sQuery.append(" AND STND_DATE BETWEEN '" + model.get("STND_DATE_F").toString() + "' AND '" + model.get("STND_DATE_T").toString() + "' ");
					} else if (key.equals("STND_DATE_T")) {
	 ...
	 * @exception
	 */
    public ModelAndView selectTermChart(Map<String, Object> model) throws Exception {
	 ...
	 * @exception
	 */
    public ModelAndView selectTermChartNew(Map<String, Object> model) throws Exception {
    	ArrayList<HashMap<String, Object>> tmpList = new ArrayList<HashMap<String, Object>>();
    ...
    private StringBuilder getChartQueryCpa(Map<String, Object> model){
		...
		for (int i=0; i<keyFieldArray.length; i++) {
			...
			if (keyFieldArray[i].equals("TIME_CD")) {
				...
				sQuery.append("dbo.FN_CODE_NM('dov_cd',DOV_CD) AS 'DOV_NM', ");
			} else if (keyFieldArray[i].equals("SHOP_ID")) {
...
//		sQuery.append("         AND WORK_YN   = 'Y') ");
	 ...
	 * @exception
	 */
    public ModelAndView selectTermChartCpa(Map<String, Object> model) throws Exception {
    	ArrayList<HashMap<String, Object>> tmpList = new ArrayList<HashMap<String, Object>>();
    	Connection conn = null;
    	Statement stmt = null;
    	try {
